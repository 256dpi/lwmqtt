...
static int net__try_connect_tcp(const char *host, uint16_t port, mosq_sock_t *sock, const char *bind_address, bool blocking)
{
	...
	ainfo_bind = NULL;
	printf("Benoit: net__try_connect_tcp\n");
...
static int net__init_ssl_ctx(struct mosquitto *mosq)
{
	...
	if(mosq->user_ssl_ctx){
		...
		if(!mosq->ssl_ctx_defaults){
			...
			printf("Benoit: ICI: net__init_ssl_ctx, compteur = %d\n", compteur++);
			printf("Error: If you use MOSQ_OPT_SSL_CTX then MOSQ_OPT_SSL_CTX_WITH_DEFAULTS must be true, or at least one of cafile, capath or psk must be specified.");
	...
	if(mosq->tls_cafile || mosq->tls_capath || mosq->tls_psk || mosq->tls_use_os_certs){
		...
		if(!mosq->tls_version){
			...
			printf("Error: Protocol %s not supported.", mosq->tls_version);
			return MOSQ_ERR_INVAL;
		}
		...
		if(mosq->tls_ciphers){
			...
			if(ret == 0){
				printf("Error: Unable to set TLS ciphers. Check cipher list \"%s\".", mosq->tls_ciphers);
				net__print_ssl_error(mosq);
				return MOSQ_ERR_TLS;
			}
		}
...
int net__socket_connect(struct mosquitto *mosq, const char *host, uint16_t port, const char *bind_address, bool blocking)
{
	...
	printf("Benoit: net__socket_connect %s\n", host);

	rc = net__try_connect(host, port, &mosq->sock, bind_address, blocking);
	if(rc > 0) return rc;

	printf("Benoit: net__socket_connect %d\n", mosq->tcp_nodelay);
	if(mosq->tcp_nodelay){
		printf("Benoit: PAS ICI: mosq->tcp_nodelay %s\n", host);
		int flag = 1;
	...
	{
		printf("Benoit: ICI: vers net__socket_connect_step3 %s\n", host);
...
/*
lwmqtt_err_t lwmqtt_mbedtls_network_connect(ssl_network_t *network, char *host, int port) {
	lwmqtt_err_t retVal = LWMQTT_SUCCESS;
