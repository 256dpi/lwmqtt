 ...
 * Will send a publish packet and wait for all acks to complete. If the encoded packet is bigger than the write buffer
 * the function will return LWMQTT_BUFFER_TOO_SHORT without attempting to send the packet.
 * If "dup" is set to true, it will mark the message as duplicate and publish it using the packet id passed (pointed by
 * "id"). If "dup" is set to false, it will mark the message as NOT duplicate and publish it with the corresponding next
 * packet id, which will be stored in the variable pointed by "id" in case the publishing fails and it has to be
 * republished it as DUP (with the same packet id).
 *
 * Note: The message callback might be called with incoming messages as part of this call.
 *
 * @param client - The client object.
