...
static int lwmqtt_read_packet(lwmqtt_client_t *c) {
  ...
  len += lwmqtt_header_encode(c->read_buf + 1, rem_len);  // put the original remaining length back into the buffer
...
static int lwmqtt_cycle(lwmqtt_client_t *c) {
  ...
  switch (packet_type) {
    ...
    case LWMQTT_PUBREC_PACKET: {
      ...
      if (lwmqtt_deserialize_ack(&type, &dup, &packet_id, c->read_buf, c->read_buf_size) != 1) {
        rc = LWMQTT_FAILURE;
...
int lwmqtt_client_publish(lwmqtt_client_t *c, const char *topicName, lwmqtt_message_t *message) {
  ...
  if (message->qos == LWMQTT_QOS1) {
    if (lwmqtt_cycle_until(c, LWMQTT_PUBACK_PACKET) == LWMQTT_PUBACK_PACKET) {
      unsigned short packet_id;
      unsigned char dup, type;
      if (lwmqtt_deserialize_ack(&type, &dup, &packet_id, c->read_buf, c->read_buf_size) != 1) rc = LWMQTT_FAILURE;
    } else {
      rc = LWMQTT_FAILURE;
    }
